...
async function checkGempa() {
    try {
        ...
        let apiResponse = await axios.get('https://api.siputzx.my.id/api/info/bmkg')
...
async function checkSholat() {
    try {
        ...
        for (let i = 0; i < jadwalsholat.length; i++) {
            ...
            if (now == prayerTime && bot.jadwalsholat.now !== currentPrayer) {
                ...
                for (let chatId of groups) {
                    try {
                        if (chat[chatId].notifazan && chat[chatId]?.sholatNow !== currentPrayer) {
                            ...
                            await createAzanImage(`Azan ${currentPrayer} Sudah Berkumandang`, "Untuk wilayah Jakarta dan sekitarnya.")
                            let thumbnail = fs.readFileSync("./media/adzan.png")
                            let { reminders } = JSON.parse(fs.readFileSync('./json/reminders.json', 'utf-8'))
                            await conn.adReply(chatId, reminders.getRandom().message, `Azan ${currentPrayer} Sudah Berkumandang`, 'Untuk wilayah Jakarta dan sekitarnya.', thumbnail, global.config.website, false)
                            await delay(2000)
                        }
                    } catch (error) {
        ...
        // console.log("Berhasil melakukan checkSholat!")
    } catch (error) {
