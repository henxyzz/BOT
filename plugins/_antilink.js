...
if (chat.antiLinkGc && m.isGroup) {
  if (isGroupLink && !isAdmin) {
    ...
    if (chat.teks) {
      ...
      await m.reply(`_*乂 Link Group Terdeteksi!*_ Pesan kamu akan dihapus!`)
    }
    await conn.sendMessage(m.chat, { delete: m.key }) 
  }
}

if (chat.antiLinkGc2 && m.isGroup) {
  if (isGroupLink && !isAdmin) {
    if (isBotAdmin) {
      const linkThisGroup = `https://chat.whatsapp.com/${await this.groupInviteCode(m.chat) ? await this.groupInviteCode(m.chat) : '123'}` 
      if (m.text.includes(linkThisGroup)) return !0 
...
if (chat.antiLinkWa && m.isGroup) {
  if (isLinkWa && !isAdmin) {
    if (chat.teks) {
      if (setting.composing) await this.sendPresenceUpdate('composing', m.chat)
      if (setting.autoread) await this.readMessages([m.key])
      await m.reply(`_*乂 Link Whatsapp Terdeteksi!*_ Pesan kamu akan dihapus!`)
    }
    await conn.sendMessage(m.chat, { delete: m.key }) 
  }
}
...
if (chat.antiLinks && m.isGroup) {
  if (isLink && !isAdmin) {
    if (chat.teks) {
      if (setting.composing) await this.sendPresenceUpdate('composing', m.chat)
      if (setting.autoread) await this.readMessages([m.key])
      await m.reply(`_*乂 Link Terdeteksi!*_ Pesan kamu akan dihapus!`)
    }
    await conn.sendMessage(m.chat, { delete: m.key }) 
  }
}