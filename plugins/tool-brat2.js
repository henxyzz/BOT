...
let handler = async (m, {
    ...
    try {
        ...
        for (let i = 0; i < words.length; i++) {
            ...
            const res = await Func.fetchBuffer(`https://brat.dev-bot.my.id/brat?text=${text}`);
        ...
        const stik = await createSticker(false, outputVideoPath, info.author, info.packname, 20)
...
async funchandlern createSticker(img, url, packName, authorName, quality) {
    let stickerMetadata = {
        type: 'crop',
        pack: packName,
        author: authorName,
